@section('scripts')
<script>
const form = document.getElementById('kt_account_add_view_form');
...
const submitButton = document.getElementById('kt_account_add_submit');
    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">{{ __('Form Tambah Perkiraan') }}</h3>
            </div>
        ...
        <div id="kt_savings_add_view">
            <form id="kt_account_add_view_form" class="form" method="POST" action="{{ route('account.process-add') }}" enctype="multipart/form-data">
            @csrf
            @method('POST')
                <div class="card-body border-top p-9">
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="account_code" class="form-control form-control-lg form-control-solid" placeholder="No. Perkiaraan" value="{{ old('account_code', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        <label class="col-lg-4 col-form-label fw-bold fs-6 required">{{ __('Nama Perkiraan') }}</label>
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="account_name" class="form-control form-control-lg form-control-solid" placeholder="Nama Perkiraan" value="{{ old('account_name', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                        <label class="col-lg-4 col-form-label fw-bold fs-6 required">{{ __('Golongan Perkiraan') }}</label>
                        <div class="col-lg-8 fv-row">
                            <input type="text" name="account_group" class="form-control form-control-lg form-control-solid" placeholder="Golongan Perkiraan" value="{{ old('account_group', '' ?? '') }}" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="row mb-6">
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    ...
                    <button type="submit" class="btn btn-primary" id="kt_account_add_submit">
